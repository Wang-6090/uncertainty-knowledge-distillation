...
$OutputRoot = ".\analysis\revised_cuda_multiseed_v3"
...
foreach ($seed in $Seeds) {
    foreach ($method in @(
        ...
        foreach ($clusterMode in @("oracle", "auto")) {
            $detectRun = ".\runs\revised_cuda_ms_s${seed}_${method}_v3_detect_${clusterMode}"
            ...
            Invoke-Python train.py discover --dataset cifar100 --data-root $DataRoot --download `
                ...
                --candidate-purify uncertainty_consensus --candidate-keep-ratio 0.75 `
...
Invoke-Python analyze_cuda_multiseed.py `
    --root .\runs `
    --glob "revised_cuda_ms_s*_v3_detect_*" `
    --out-dir $OutputRoot

Write-Host "CUDA detection v3 finished."
