...
$BaseRun = ".\runs\cifar_margin_uncertainty"
...
if (-not (Test-Path $TeacherCkpt)) {
    python train.py train_teacher --dataset cifar100 --data-root $DataRoot --num-known 60 --seed 42 --split-path $SplitPath --image-size 64 --batch-size 32 --num-workers 0 --backbone resnet18 --pretrained --epochs 15 --alpha-proto 0.1 --alpha-pseudo 0.0 --uncertainty-target-mode margin --work-dir $BaseRun --teacher-ckpt $TeacherCkpt --device auto
    if ($LASTEXITCODE -ne 0) { throw "margin teacher training failed" }
}

if (-not (Test-Path $StudentCkpt)) {
    python train.py train_student --dataset cifar100 --data-root $DataRoot --num-known 60 --seed 42 --split-path $SplitPath --image-size 64 --batch-size 32 --num-workers 0 --backbone resnet18 --pretrained --epochs 15 --alpha-proto 0.1 --alpha-pseudo 0.0 --uncertainty-target-mode margin --work-dir $BaseRun --teacher-ckpt $TeacherCkpt --student-ckpt $StudentCkpt --device auto
    if ($LASTEXITCODE -ne 0) { throw "margin student training failed" }
}

$modes = @("full", "entropy_aleatoric", "entropy_proto")
foreach ($mode in $modes) {
    $runDir = ".\runs\cifar_margin_uncertainty_$mode"
    ...
    if ($LASTEXITCODE -ne 0) { throw "margin discovery failed for mode=$mode" }
}

Write-Host "margin uncertainty experiment finished."
