...
foreach ($seed in $Seeds) {
    $teacherRun = ".\runs\revised_cuda_ms_s${seed}_teacher"
    $teacherCkpt = Join-Path $teacherRun "teacher.pt"
    if (-not (Test-Path $teacherCkpt)) {
        Invoke-Python train.py train_teacher --dataset cifar100 --data-root $DataRoot --download `
            --num-known 60 --seed $seed --split-path $SplitPath `
            --image-size 64 --batch-size 64 --num-workers 0 `
            --backbone resnet34 --teacher-backbone resnet34 --pretrained `
            --epochs $Epochs --alpha-unc 0.1 --alpha-proto 0.0 --alpha-pseudo 0.0 `
            --work-dir $teacherRun --teacher-ckpt $teacherCkpt --device cuda
    } else {
        Write-Host "skip existing teacher: $teacherCkpt"
    }

    foreach ($exp in $experiments) {
        $studentRun = ".\runs\revised_cuda_ms_s${seed}_$($exp.Name)"
        $studentCkpt = Join-Path $studentRun "student.pt"
        if (-not (Test-Path $studentCkpt)) {
            Invoke-Python train.py train_student --dataset cifar100 --data-root $DataRoot --download `
                --num-known 60 --seed $seed --split-path $SplitPath `
                --image-size 64 --batch-size 64 --num-workers 0 `
                --backbone resnet18 --teacher-backbone resnet34 --student-backbone resnet18 --pretrained `
                --epochs $Epochs --alpha-unc $exp.Unc --alpha-kd $exp.KD --kd-mode $exp.UKD `
                --alpha-feat-kd $exp.Feat --alpha-supcon $exp.SupCon --alpha-proto $exp.Proto --alpha-pseudo 0.0 `
                --work-dir $studentRun --teacher-ckpt $teacherCkpt --student-ckpt $studentCkpt --device cuda
        } else {
            Write-Host "skip existing student: $studentCkpt"
        }

        foreach ($clusterMode in @("oracle", "auto")) {
            $detectRun = ".\runs\revised_cuda_ms_s${seed}_$($exp.Name)_detect_$clusterMode"
...
Invoke-Python analyze_cuda_multiseed.py --root .\runs --glob "revised_cuda_ms_s*_detect_*" --out-dir .\analysis\revised_cuda_multiseed
Write-Host "CUDA multi-seed revised ablation finished."
