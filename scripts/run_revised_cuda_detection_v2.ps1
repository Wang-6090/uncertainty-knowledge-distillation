...
$OutputRoot = ".\analysis\revised_cuda_multiseed_v2"
...
foreach ($seed in $Seeds) {
    foreach ($method in @(
        ...
        foreach ($clusterMode in @("oracle", "auto")) {
            $detectRun = ".\runs\revised_cuda_ms_s${seed}_${method}_v2_detect_${clusterMode}"
...
Invoke-Python analyze_cuda_multiseed.py `
    --root .\runs `
    --glob "revised_cuda_ms_s*_v2_detect_*" `
    --out-dir $OutputRoot

Write-Host "CUDA detection v2 finished."
