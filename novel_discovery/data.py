...
def build_data_bundle(
    ...
    if dataset_name.lower() == "imagefolder":
        # Support datasets such as CIFAR-100-dataset-main, which contain
        # separate train/ and test/ ImageFolder trees. Falling back to root
        # keeps compatibility with a single-tree ImageFolder dataset.
        root_path = Path(root)
        ...
        train_base = datasets.ImageFolder(root=str(train_root))
        all_classes = list(train_base.classes)
        ...
        known_classes, novel_classes = make_class_split(all_classes, num_known, seed, imagefolder_split)
        train_tf = build_transforms(image_size, train=True)
        test_tf = build_transforms(image_size, train=False)
