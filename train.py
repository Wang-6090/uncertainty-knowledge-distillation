...
def discover(args):
    ...
    result["inference_samples_per_second"] = float(
        len(outputs_test["labels"]) / max(inference_seconds, 1e-9)
    )
    calibration_report["inference_seconds"] = float(inference_seconds)
    run_dir = ensure_dir(args.work_dir)
